...
## Project Overview
Convert the existing Ukho Network web application (React + Vite) into a native mobile application using React Native and Expo. The application is a social network platform focused on connecting South African tribal communities, clans, and lineages. Every visual element, component, interaction, and styling must be replicated identically to the web version.
